...
to_email_input = st.text_input("Recipient Email", "recipient@example.com")
...
if st.button("Generate and Send Email"):
    ...
    def send_html_email(subject: str, html_body: str) -> Dict[str, str]:
        ...
        st.session_state['last_email_body'] = html_body
    ...
    async def main():
        with trace("Automated SDR for Visexa AI"):
            with st.spinner("Generating, formatting, and sending email..."):
                # The result is still captured but not displayed
                result = await Runner.run(sales_manager, message)
                st.success(f"Email process completed successfully for {to_email_input}!")

                # Display the email content from session_state
                if 'last_email_subject' in st.session_state and 'last_email_body' in st.session_state:
                    with st.expander("Show Sent Email Content"):
                        st.subheader("Subject")
                        st.write(st.session_state['last_email_subject'])
                        st.subheader("Body")
                        st.markdown(st.session_state['last_email_body'], unsafe_allow_html=True)

    # Clear previous run's email data before starting a new one
    if 'last_email_subject' in st.session_state:
        del st.session_state['last_email_subject']
    if 'last_email_body' in st.session_state:
        del st.session_state['last_email_body']
